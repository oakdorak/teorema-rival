...
class IntegradorAudio:
    def __init__(self):
        ...
        # Obtenemos la URL base de la app (Vercel la provee como VERCEL_URL)
        self.base_url = os.getenv("VERCEL_URL", "localhost:8000")
        if not self.base_url.startswith("http"):
            self.base_url = f"https://{self.base_url}"

        # Rutas internas a los audios de referencia (están en la carpeta /public/refs/)
    ...
    def generar_audio(self, texto, voice="alloy", filename_prefix="batalla"):
        ...
        """
        if voice == "alloy":
            ref_audio = self.ref_teorema_url
            ref_text = self.text_teorema
        else:
            ref_audio = self.ref_goku_url
        ...
        try:
            ...
            # Result es una tupla: (audio_path, status)
            audio_url = result[0]
            
            # Descargamos el audio generado
